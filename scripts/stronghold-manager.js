...
export class StrongholdManager extends Application {
  ...
  async getData() {
    ...
    const strongholdArray = Object.values(strongholds).map((stronghold) => {
      const upgradeCost =
        stronghold.level < 5
          ? StrongholdData.getUpgradeCost(stronghold.level, stronghold.level + 1)
          : 0;
      const typeSummary = StrongholdData.getTypeMechanicsSummary(stronghold.type);
      const classSummary = stronghold.classFlavor
        ? StrongholdData.getClassMechanicsSummary(stronghold.classFlavor)
        : { followers: '', actions: [], tables: [] };
      return {
        ...stronghold,
        ...
        typeSummary,
        classSummary,
        classFlavorDisplay: stronghold.classFlavor
          ? StrongholdData.CLASS_FLAVOR_DISPLAY[stronghold.classFlavor] || stronghold.classFlavor
          : null,
      };
    });
    ...
    return {
      ...
      enableAutoApplyBonuses: game.settings.get(
        'strongholds-and-followers',
        'enableAutoApplyBonuses',
      ),
  ...
  activateListeners(html) {
    ...
    html.find('.save-prices').click(async (ev) => {
      ...
      const bClean = Object.fromEntries(
        Object.entries(building).filter(([, v]) => Number.isFinite(v)),
      );
      const uClean = Object.fromEntries(
        Object.entries(upgrading).filter(([, v]) => Number.isFinite(v)),
      );
  ...
  async _onCreateStronghold(event) {
    ...
    const dialog = new Dialog({
      ...
      content: await renderTemplate(
        'modules/strongholds-and-followers/templates/stronghold-form.hbs',
        {
          stronghold: {},
          strongholdTypes: StrongholdData.STRONGHOLD_TYPES,
          classFlavors: StrongholdData.CLASS_FLAVORS,
      ...
      render: (html) => {
        try {
          html.closest('.app').style.minWidth = '600px';
        } catch {}
      },
      buttons: {
        create: {
  ...
  async _createStronghold(name, type, classFlavor, level, description) {
    const strongholds = game.settings.get('strongholds-and-followers', 'strongholds');
    const newStronghold = StrongholdData.createStronghold(
      name,
      type,
      classFlavor,
      level,
  ...
  async _onToggleStronghold(event) {
    ...
    ui.notifications.info(
      `${stronghold.active ? 'Activated' : 'Deactivated'} stronghold: ${stronghold.name}`,
    );
    this.render();
  }
  ...
  async _onUpgradeStronghold(event) {
    ...
    if (confirmed) {
      ...
      ui.notifications.info(
        `Upgraded ${stronghold.name} to Level ${stronghold.level} (Cost: ${upgradeCost.toLocaleString()} gp)`,
      );
      this.render();
    }
  }
