...
Hooks.once('ready', async function () {
  ...
  if (game.system.id !== 'dnd5e') {
    ui.notifications.warn(
      'Strongholds & Followers: This module is designed for the D&D 5e system. Some features may not work correctly.',
    );
  ...
  if (game.user.isGM) {
    ui.notifications.info(
      "Strongholds & Followers module loaded. Click the 'Strongholds' button in the scene controls toolbar.",
    );
  }
});
...
Hooks.on('dnd5e.restCompleted', async (actor, data) => {
  ...
  if (hasBonuses) {
    ...
    console.log(
      `Strongholds & Followers | Applied bonuses to ${actorName} (${actorClasses.join(', ')})`,
    );
  }
});
