...
export class StrongholdData {
  static readonly STRONGHOLD_TYPES: StrongholdType[] = ['temple', 'keep', 'tower', 'establishment'];
  ...
  static readonly STRONGHOLD_TYPE_DESCRIPTIONS: Record<StrongholdType, string> = {
    ...
    temple:
      'A sacred site of worship and healing. Offers rites, blessings, sanctuary, and faithful acolytes.',
    tower:
      'A seat of arcane study and experimentation. Enables research, spell scribing, wards, and curious constructs.',
    establishment:
      'A hub of commerce and influence. Generates income, rumors, contacts, and favors from patrons.',
  } as const;
  ...
  static readonly TYPE_MECHANICS_SUMMARY: Record<
    StrongholdType,
    { demesne: string[]; actions: string[]; notes: string[] } | undefined
  > = {
    keep: {
      demesne: [
  ...
  static readonly CLASS_FLAVOR_DESCRIPTIONS: Record<ClassFlavor, string> = {
    ...
    druid:
      'A sacred grove tended by circles, where nature spirits are invoked and seasons observed.',
    ...
    sorcerer:
      'A personal sanctum attuned to bloodline magic, experiments, and unpredictable power.',
  ...
  static readonly CLASS_MECHANICS_SUMMARY: Record<
    ClassFlavor,
    { followers: string; actions: string[]; tables: string[] } | undefined
  > = {
    barbarian: {
  ...
  static getCostConfig(): CostConfig {
    ...
    const buildingOverride = (F.settings()?.get('strongholds-and-followers', 'buildingCosts') ??
      {}) as Partial<CostConfig['building']>;
    const upgradeOverride = (F.settings()?.get('strongholds-and-followers', 'upgradeCosts') ??
      {}) as Partial<CostConfig['upgrading']>;
    return {
  ...
  static createStronghold(
    ...
    const id =
      (globalThis as any).foundry?.utils?.randomID?.() ?? crypto.randomUUID?.() ?? `${Date.now()}`;
    return {
      id,
      name,
      type,
  ...
  static addBonus(
    ...
    strongholdId: string,
    bonus: Omit<StrongholdBonus, 'id' | 'addedDate'>,
  ): Record<string, Stronghold> {
  ...
  static getCharacterName(actor: any): string {
  ...
  static getApplicableBonuses(stronghold: Stronghold, actor: any): StrongholdBonus[] {
    ...
    return custom.filter(
      (b) =>
        b.partyWide ||
        (!!stronghold.classFlavor && this.actorHasMatchingClass(actor, stronghold.classFlavor)) ||
        (!b.partyWide && !stronghold.classFlavor),
    );
  }
}
